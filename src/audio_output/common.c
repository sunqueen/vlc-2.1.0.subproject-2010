...
unsigned aout_CheckChannelReorder( const uint32_t *chans_in,
                                   ...
                                   uint32_t mask, uint8_t *__restrict table )			// sunqueen modify
{
    unsigned channels = 0;
...
void aout_ChannelReorder( void *ptr, size_t bytes, unsigned channels,
                          const uint8_t *__restrict chans_table, vlc_fourcc_t fourcc )			// sunqueen modify
{
    assert( channels != 0 );
...
\
    ...
    switch( fourcc )
    {
        ...
        default:
        {
            ...
            unsigned char *buf = (unsigned char *)ptr;			// sunqueen modify
...
void aout_Interleave( void *__restrict dst, const void *const *srcv,
                      unsigned samples, unsigned chans, vlc_fourcc_t fourcc )			// sunqueen modify
{
#define INTERLEAVE_TYPE(type) \
do { \
    type *d = (type *)dst; \
    for( size_t i = 0; i < chans; i++ ) { \
        const type *s = (type *)srcv[i]; \
                          ...
                          int i_sample_count, const int *pi_selection, int i_bits_per_sample )
{
    ...
    if( i_bits_per_sample == 8 )
        ExtractChannel( (uint8_t *)p_dst, i_dst_channels, (const uint8_t *)p_src, i_src_channels, i_sample_count, pi_selection, 1 );				// sunqueen modify
    else  if( i_bits_per_sample == 16 )
        ExtractChannel( (uint8_t *)p_dst, i_dst_channels, (const uint8_t *)p_src, i_src_channels, i_sample_count, pi_selection, 2 );				// sunqueen modify

    else  if( i_bits_per_sample == 32 )
        ExtractChannel( (uint8_t *)p_dst, i_dst_channels, (const uint8_t *)p_src, i_src_channels, i_sample_count, pi_selection, 4 );				// sunqueen modify
    else  if( i_bits_per_sample == 64 )
        ExtractChannel( (uint8_t *)p_dst, i_dst_channels, (const uint8_t *)p_src, i_src_channels, i_sample_count, pi_selection, 8 );				// sunqueen modify
}
                              ...
                              const char *psz_name, bool b_add )
{
    ...
    const char **ppsz_filter = (const char **)calloc( i_count, sizeof(*ppsz_filter) );			// sunqueen modify
    if( !ppsz_filter )
    {
        free( psz_list );
        return false;
    ...
    char *psz_new = (char *)malloc( i_length + 1 );			// sunqueen modify
    *psz_new = '\0';
    for( int i = 0; i < i_count; i++ )
    {
        if( *ppsz_filter[i] == '\0' )
