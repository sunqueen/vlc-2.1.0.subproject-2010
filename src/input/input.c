                               ...
                               input_resource_t *p_resource )
{
    ...
    _snprintf( psz_timer_name, sizeof(psz_timer_name),
              "input launching for '%s'", psz_name );			// sunqueen modify
...
static void LoadSubtitles( input_thread_t *p_input )
{
    ...
    for( int i = 0; i < p_input->p->i_attachment; i++ )
    {
        ...
        if( !strcmp( a->psz_mime, "application/x-srt" ) )
            TAB_APPEND( (input_attachment_t **), i_attachment, pp_attachment,
                        vlc_input_attachment_New( a->psz_name, NULL,
                                                  a->psz_description, NULL, 0 ) );			// sunqueen modify
    }
    vlc_mutex_unlock( &p_input->p->p_item->lock );
                             ...
                             input_source_t *p_source, vlc_meta_t *p_meta )
{
    ...
    demux_meta_t *p_demux_meta = (demux_meta_t *)
        vlc_custom_create( p_demux, sizeof( *p_demux_meta ), "demux meta" );			// sunqueen modify
    if( !p_demux_meta )
        return;
                              ...
                              int i_new, input_attachment_t **pp_new )
{
    ...
    attachment = (input_attachment_t **)xrealloc( attachment,
                    sizeof(*attachment) * ( i_attachment + i_new ) );			// sunqueen modify
    for( i = 0; i < i_new; i++ )
