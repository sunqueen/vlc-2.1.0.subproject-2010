    ...
    if( !strstr( psz_uri, "://" )
     || strchr( psz_uri, ' ' ) || strchr( psz_uri, '"' ) )
        fprintf( stderr, "Warning: %s(\"%s\"): file path instead of URL.\n",
                 __FUNCTION__, psz_uri );				// sunqueen modify
#endif
    vlc_mutex_lock( &p_i->lock );
    free( p_i->psz_uri );
                          ...
                          unsigned flags )
{
    ...
    INSERT_ELEM( (char **), p_input->ppsz_options, p_input->i_options,
                 p_input->i_options, strdup( psz_option ) );			// sunqueen modify
out:
                               ...
                               const char *psz_format, va_list args )
{
    ...
    if( !p_cat )
    {
        ...
        if( !p_cat )
            return VLC_ENOMEM;
        INSERT_ELEM( (info_category_t **), p_i->pp_categories, p_i->i_categories, p_i->i_categories,
                     p_cat );				// sunqueen modify
    }
...
void input_item_ReplaceInfos( input_item_t *p_item, info_category_t *p_cat )
{
    ...
    {
        INSERT_ELEM( (info_category_t **), p_item->pp_categories, p_item->i_categories, p_item->i_categories,
                     p_cat );				// sunqueen modify
    }
    vlc_mutex_unlock( &p_item->lock );
...
void input_item_SetEpg( input_item_t *p_item, const vlc_epg_t *p_update )
{
    ...
    for( int i = 0; i < p_epg->i_event; i++ )
    {
        ...
        _snprintf( psz_start, sizeof(psz_start), "%4.4d-%2.2d-%2.2d %2.2d:%2.2d:%2.2d",
                  1900 + tm_start.tm_year, 1 + tm_start.tm_mon, tm_start.tm_mday,
                  tm_start.tm_hour, tm_start.tm_min, tm_start.tm_sec );				// sunqueen modify
...
static int GuessType( const input_item_t *p_item )
{
    ...
    e = (const item_type_entry *)bsearch( p_item->psz_uri, tab, sizeof( tab ) / sizeof( tab[0] ),
                 sizeof( tab[0] ), typecmp );				// sunqueen modify
...
void input_item_node_Delete( input_item_node_t *p_node )
{
    if( p_node->p_parent )
        for( int i = 0; i < p_node->p_parent->i_children; i++ )
            ...
            {
                REMOVE_ELEM( (input_item_node_t **), p_node->p_parent->pp_children,
...
void input_item_node_AppendNode( input_item_node_t *p_parent, input_item_node_t *p_child )
{
    ...
    INSERT_ELEM( (input_item_node_t **),
				 p_parent->pp_children,
                 p_parent->i_children,
                 p_parent->i_children,
                 p_child );				// sunqueen modify
