...
{
    image_handler_t *p_image = (image_handler_t *)calloc( 1, sizeof(image_handler_t) );				// sunqueen modify
    if( !p_image )
        return NULL;
...
static decoder_t *CreateDecoder( vlc_object_t *p_this, video_format_t *fmt )
{
    ...
    p_dec = (decoder_t *)vlc_custom_create( p_this, sizeof( *p_dec ), "image decoder" );			// sunqueen modify
    if( p_dec == NULL )
        return NULL;
                               ...
                               const char *psz_module )
{
    ...
    p_filter = (filter_t *)vlc_custom_create( p_this, sizeof(filter_t), "filter" );			// sunqueen modify
